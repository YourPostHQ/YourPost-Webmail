...
export default function SystemLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (!isAuthenticated() || !isSystem()) router.replace('/login')
  }, [router])
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-zinc-50 dark:bg-zinc-900">
      ...
      {/* ── Sidebar ── */}
      <aside className="w-56 flex-shrink-0 flex flex-col h-screen bg-red-950 border-r border-red-900">

        {/* Brand */}
        <div className="px-4 pt-5 pb-4 border-b border-red-900 flex-shrink-0">
          <Link href="/system" className="flex items-center gap-2 text-white mb-3">
            <span dangerouslySetInnerHTML={{ __html: logoSvg }} />
            <span className="font-bold text-[15px]">YourPost</span>
          </Link>
          <span className="px-2 py-0.5 text-[11px] font-semibold bg-red-700 text-white rounded">System</span>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-0.5">
          {navItems.map(item => {
            ...
            return item.disabled ? (
              <div
                key={item.label}
                className="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm text-red-900 cursor-default select-none"
              >
                <span className="opacity-40 text-red-300">{item.icon}</span>
                <span className="opacity-40 text-red-300">{item.label}</span>
                <span className="ml-auto text-[10px] text-red-800 opacity-80">soon</span>
              </div>
            ) : (
              <Link
                key={item.label}
        ...
        {/* Bottom */}
        <div className="flex-shrink-0 border-t border-red-900 p-3 flex items-center justify-between">
          <Link
            href="/inbox"
            className="flex items-center gap-1.5 text-xs text-red-400 hover:text-red-100 transition-colors"
          >
          ...
          <button
            onClick={toggleTheme}
            className="p-1.5 rounded text-red-400 hover:text-red-100 transition-colors"
